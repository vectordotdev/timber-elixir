  ...
  describe "Mix.Tasks.Timber.Install.API.wait_for_logs/2" do
    test "204" do
      FakeHTTPClient.stub(:request, fn
        (:get, [{'Authorization', 'Basic YXBpX2tleQ=='}, {'X-Installer-Session-Id', 'session_id'}], "https://api.timber.io/installer/has_logs", []) -> {:ok, 204, ""}
        (:post, [{'Authorization', 'Basic YXBpX2tleQ=='}, {'X-Installer-Session-Id', 'session_id'}], "https://api.timber.io/installer/events", [body: %{event: %{data: nil, name: :waiting_for_logs}}]) -> {:ok, 204, ""}
      end)
